...
int main(){ 
    ...
    while(tc--){
        ...
        for(int i = 0 ; i < n ; ++i){
            ...
            points.push_back({b.x, 0});
            LS[i].first = LineSegment(a, b);
            LS[i].second = cost;
        }
        sort(LS, LS + n);
        if(points.empty()){
            cout << "-inf +inf 1.000" << endl;
            continue;
        }
        sort(points.begin(), points.end());
        cout << points.size() + 1 << endl;
        cout  << "-inf " << points[0].x << " 1.000" << endl;
        for(int i = 0 ; i < points.size() ; ++i){
            point inp = points[i];
            inp.x += 0.0001;
            double ans = 1.0;
            for(int j = 0 ; j < n ; ++j){
                if(inp.x >= LS[j].first.p.x &&  inp.x <= LS[j].first.q.x ){
                    ans *= LS[j].second;
                }
            }
            if(i + 1 < points.size()) 
                cout  << points[i].x << " " << points[i + 1].x << " " << ans << endl;
            else
                cout  << points[i].x << " " << "+inf " << ans << endl;
        }
        if(cnt == 1 && tc != 0){
            cout << endl;
            cnt = 0;
