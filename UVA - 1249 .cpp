...
int main(){ 
    ...
    while(1){
        ...
        cin >> E.x >> E.y;
        cin >> F.x >> F.y;
        if(A == B && B == C && C == D && D == E && E == F && A.x == A.y && A.y == 0.0){
            break;
        }
        point G,H;
        double A1 = traingleArea(D, E, F);
        double AH = A1 / ( dist(A, B) * sin(acos(dot(toVec(A, B), toVec(A, C)) / (dist(A, B) * dist(A, C)))));
        vec AC_unit = scale(toVec(A, C), 1.0/dist(A, C));
        AC_unit = scale(AC_unit, AH);
        H = translate(A, AC_unit);
        G = translate(B, AC_unit);
