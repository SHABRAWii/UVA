...
#define slope(_point1, _point2) (double)((_point2.y - _point1.y) / (_point2.x - _point1.x))
...
int main(){ 
    ...
    while(1){
        ...
        if(cin.eof())
            break;
        point squareCenter((a.x + c.x) / 2.0, (a.y + c.y) / 2.0);
        vec sqC_c = toVec(squareCenter, c); // vector from center to c
        swap(sqC_c.x, sqC_c.y); // This is            rotate vector
        sqC_c.x *=-1;           //           how you                 90 degree
        point b(translate(squareCenter, sqC_c));
        point d(translate(squareCenter, scale(sqC_c, -1)));// with vector rotated -90
